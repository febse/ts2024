...
```{r}
# Create a sound from a sine wave

sine_wave <- sine(500, duration = 1, samp.rate = 200, xunit = "time")
```
...
play(sine_wave, player = "cvlc")
```

```{r}
# Plot the sound in the time domain

plot(sine_wave, main = "Sine wave in the time domain")
```
