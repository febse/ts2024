...
The lag polynomial representation of the difference equation is:
...
```{r}
# Type your code here
solutions <- polyroot(c(3, -1, 1))
solutions
```

```{r}
# Check that you have found the correct roots
...
where $I$ is the identity matrix (with such a dimension that the operations are valid).


<!-- ## Complex Roots of the Characteristic Equation

As was the case in the exercise above, it is possible that the roots of the characteristic equation are complex numbers. Complex roots imply a cyclical behavior in the system. To see that, you need to know what the multiplication of complex numbers does.

Let $z_1 = a + bi$ and $z_2 = c + di$ be two complex numbers. The product of these two numbers is:
...
```{r}
# Plot the three complex numbers as vectors from the origin

dt <- tibble(
  x = c(Re(z1), Re(z2), Re(z_prod)),
  y = c(Im(z1), Im(z2), Im(z_prod)),
  label = c("z1", "z2", "z1 * z2")
)

ggplot(dt) +
  geom_segment(aes(x = 0, y = 0, xend = x, yend = y), arrow = arrow(length = unit(0.2, "inches"))) +
  geom_text(aes(x = x, y = y, label = label), nudge_x = 0.1, nudge_y = 0.4) +
  xlim(-10, 10) + ylim(-10, 10) +
  labs(title = "Complex Numbers", x = "Re", y = "Im") +
  theme_minimal()
``` -->
